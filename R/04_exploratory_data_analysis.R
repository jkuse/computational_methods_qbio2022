...
# correlation above 0.7 means, in a standard way, a significant correlation
# pairs() is a function to discover correlation (could be in a vector, or data.frame...)
...
table(iris$Species) #counts how many obs you have in a vector

# sepal length mean per species #index must be a list
tapply(X = iris$Sepal.Length, INDEX = list(iris$Species), FUN = mean) #mean value for each specie
# the same operation, using another function, other arguments and other output
aggregate(x = iris$Sepal.Length, by = list(iris$Species), FUN = mean) #aggregate function output a matrix
...
par(mfrow = c(2, 2)) #sets different parameters for plotting!!
...
irisWithoutOut <- iris[which(!iris$Sepal.Width %in% outliers), c("Sepal.Width", "Species")]
...
# qqnorm -> quantile plot to a particular value
# if we have a perfect normal distribution all the dots should fit de line
