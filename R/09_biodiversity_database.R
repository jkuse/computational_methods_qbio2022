...
# unique vector is used so we don't have duplicated names
...
# Checking geographical coordinates
plot(decimalLatitude ~ decimalLongitude, data = myrsine.data, asp = 1)
map(, , , add = TRUE)

# clean_coordinates(): clean the species records checks for common
# errors in coordinates such as institutional coordinates, sea coordinates,
# outliers, zeros, centroids AND DOES NOT ACCEPT NA
...
write.csv(myrsine.new.geo2,
          ...
          row.names = FALSE)
# Homework
# how to use this db to  plot in tmap? how to convert into a shapefile
