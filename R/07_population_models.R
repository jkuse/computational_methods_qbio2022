...
logGrowth <- function(t, y, p) {
  N <- y[1]
  with(as.list(p), { #with the parameters of "p" you should solve the equation
    dN.dt <- r * N * (1 - a * N)
    return(list(dN.dt))
  })
}
...
head(out_log)
class(out_log)
# creating data frame with out_log
...
# creating a function that represents the derivatives of both populations
...
# we need to put this data in a tidy format and we need to make it a df to do so
df_lv <- pivot_longer(as.data.frame(out_lv), cols = 2:3)

# channging number for name of species
df_lv$name[df_lv$name == 1] <- "Linx"
